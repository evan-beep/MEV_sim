...
node_fake = "HTTP://127.0.0.1:7545"

eth = Web3(Web3.HTTPProvider(node_url))
try:
...
def decode_transaction_input(transac):
    abi = get_contract_abi(transac['hash'])
...
latest_block = 18869414  # eth.eth.block_number
print(latest_block)

pending_transactions = eth.eth.get_block(
    latest_block, full_transactions=True).transactions

for i in range(1, 10):
    pending_transactions += eth.eth.get_block(
        latest_block-i, full_transactions=True).transactions

DEX_ADDRESSES = [
    '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f',  # uniswapv2,
    '0x1F98431c8aD98523631AE4a59f267346ea31F984',  # univ3 main,
    '0xC0AeE478e3658e2610c5F7A4A2E1777Ce9e4f2Ac',  # sushiswap main
    '0x3e8b9901dBFE766d3FE44B36c180A1bca2B9A295',  # balancer
    '0x9424B1412450D0f8Fc2255FAf6046b98213B76Bd'  # curve
]


identified_transactions = []
for tr in pending_transactions:
    if tr['to'] in DEX_ADDRESSES:
        identified_transactions.append(tr)
'''
'''
for trs in pending_transactions[:3]:
    eth_fake.eth.send_transaction({
        'to': receiver,
        'from': sender,
        'value': trs['value']
    })
...
v3_pool_address = Web3.to_checksum_address(
...
v3_pool = web3.eth.contract(abi=v3_pool_abi, address=v3_pool_address)


end_block = 18889000
start_block = end_block - 100
event_filter = v3_pool.events.Swap.create_filter(
...
for event in events:
    print('TX hash: {}'.format(event['transactionHash']))
    print(event)
